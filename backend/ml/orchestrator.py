...
MLflow via adhidebugExperiment. Returns a TrainingResult
dataclass aggregating per-model metrics, gate status,
output directory, and MLflow run ID

Connects to:
  ml/experiment         - adhidebugExperiment context manager
...
class TrainingOrchestrator:
    ...
    def __init__(
        self,
        output_dir: Path,
        experiment_name: str = "adhi-debugadhidebug-training",
        epochs: int = 100,
        batch_size: int = 256,
    ) -> None:
    ...
    def run(
        ...
        with adhidebugExperiment(self._experiment_name) as experiment:
