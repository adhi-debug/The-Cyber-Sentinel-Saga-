...
2. **Instrument the pipeline** with counters and histograms
   - `adhidebug_requests_processed_total` (counter by stage)
   - `adhidebug_threats_detected_total` (counter by severity)
   - `adhidebug_inference_duration_seconds` (histogram)
   - `adhidebug_queue_depth` (gauge per queue)
...
- `adhidebug.raw` - Raw log lines (partitioned by hash of source IP)
- `adhidebug.parsed` - Parsed log entries (same partition key)
- `adhidebug.enriched` - Enriched feature vectors
- `adhidebug.scored` - Final scored results for dispatch
